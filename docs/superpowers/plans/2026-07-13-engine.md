...
describe('parseLcov', () => {
  it('splits covered from uncovered lines', () => {
    expect(parseLcov(SAMPLE).get('src/math.ts')).toEqual({
      file: 'src/math.ts',
      covered: [1, 2],
      uncovered: [5, 6],
    })
  ...
  it('normalizes an absolute SF path to repo-relative', () => {
    expect(parseLcov(SAMPLE, '/home/user/proj').has('src/math.ts')).toBe(true)
  })

  it('keeps already-relative paths as they are', () => {
    expect(parseLcov(SAMPLE).get('src/util.ts')?.covered).toEqual([3])
  })
