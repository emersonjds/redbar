...
git commit -m "feat(assertions): count assertions per language, from the registry"
```
...
git commit -m "feat(agents): registry of the agents redbar knows how to drive"
```
...
git commit -m "feat(outcome): cross the before with the after, and the agent's word never becomes a verdict"
```
...
git commit -m "feat(execute): the gap-by-gap loop, with the three gates that judge what the agent wrote"
```
...
git commit -m "feat(report): report of what the agent did, with the measured separated from the claimed"
```
...
```bash
git add src/cli.ts
git commit -m "feat(cli): execute command, which hands the gaps to the agent and measures the result"
```
...
git commit -m "docs: document the execute command and the rule that the agent does not grade itself"
```
